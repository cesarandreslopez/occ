...
npm test                         # Build + run the Node test suite via tsx
...
- Tests run through Node's built-in test runner with `tsx`
...
│   ├── create-fixtures.js    # Generates document fixtures
...
- **Code exploration**: `occ code` builds an in-memory graph for each command and surfaces `resolved`, `ambiguous`, and `unresolved` relationship states
